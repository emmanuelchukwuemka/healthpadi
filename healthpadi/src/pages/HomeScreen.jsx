...
const HomeScreen = () => {
  ...
  const recentActivity = [
    ...
    { id: 2, type: "consultation", date: "Yesterday, 2:15 PM", doctor: "Dr. Adebayo" }
  ];
            ...
            <div key={activity.id} className="activity-item">
              <div className="activity-content">
                {activity.type === "symptom" ? (
                  <p>Symptom logged: <strong>{activity.symptom}</strong> on {activity.date}</p>
                ) : (
                  <p>Consultation with <strong>{activity.doctor}</strong> on {activity.date}</p>
                )}
              </div>
            </div>
          ))}
